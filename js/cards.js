    ...
    .map((p) => {
      ...
      return `
        <article class="card product-card">
          ...
          <h3>${p.name}</h3>
          <div class="rating-row"><span class="stars">${stars(p.rating)}</span><span>${p.rating}</span></div>
          <p>${p.summary}</p>
          <div class="price-row">
            <div><span class="price-label">From</span><div class="price">${money(p.price)}</div></div>
            <span class="category-pill">${p.bestFor}</span>
          </div>
          <div class="spec-chips"><span>${p.ram}</span><span>${p.storage}</span><span>${p.processor}</span></div>
