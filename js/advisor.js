function getBudget(text) {
  const match = text.match(/(?:under|max|budget)\s*\$?(\d+)/);
...
function scoreLaptop(product, query) {
  ...
  if (!reasons.length) reasons.push("High overall value score.");

  return { ...product, advisorScore, reasons };
}
...
function runAdvisor() {
  ...
  if (!query) {
    ...
    result.innerHTML = `<h3>Tell us what you need</h3><p>Try: <strong>I need a laptop under $900 for college and programming.</strong></p>`;
    return;
  }
      ...
      ${recommendations.map((laptop, index) => `
        <article class="advisor-card">
          ...
          <div>
            ...
            <ul>${laptop.reasons.slice(0, 3).map((reason) => `<li>${reason}</li>`).join("")}</ul>
